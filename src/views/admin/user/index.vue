    ...
    methods: {
      ...
      async handleUpdate(row) {
        ...
        detail(id).then(response => {
          ...
          let {roleIds,sysUser} = response.data
      ...
      handleSubmit: function () {
        this.$refs['form'].validate(valid => {
          if (valid) {
            ...
            if (id != undefined) {
              update(this.form.id,{sysUser:this.form,roleIds:this.form.roleIds}).then(() => {
                this.$message.success('修改成功')
                this.dialog.visible = false
                this.handleQuery()
              })
            } else {
              add({sysUser:this.form,roleIds:this.form.roleIds}).then(() => {
